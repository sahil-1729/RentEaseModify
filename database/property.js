  ...
  try {
    ...
    if (search) {
      ...
      searchQuery["address.city"] = {
        $regex: new RegExp(search.split("(")[0], "i"),
      };
    }
    ...
    // const Properties = await Property.filter((val)=>{
    //     searchQuery}).sort(sortCriteria);
...
Router.get("/api/property/:id", async (req, res) => {
  try {
    ...
    if (!findToken) {
      ...
      res.cookie("propertyViewsToken", token, {
        httpOnly: true,
...
Router.post("/api/property/:id", authenticate, async (req, res) => {
  try {
    ...
    const existingComment = property.comments.find(
